  ...
  module ResolutionsData
    RSpec.describe MinutesBuilder do
      ...
      describe "#add_section and #build" do
        it "builds an Edoxen::Minutes with all sections" do
          builder = described_class.new(identifier: identifier, urn: urn, language_code: "eng")
          ...
          expect(minutes.language_code).to eq("eng")
          expect(minutes.script).to eq("Latn")
          expect(minutes.sections.size).to eq(2)
          expect(minutes.sections.first.number).to eq("1")
          expect(minutes.sections.first.title).to eq("Opening remarks")
        ...
        it "supports find_section via the underlying model" do
          ...
          minutes = builder.build
          expect(minutes.find_section("14.2")&.title).to eq("Financial matters")
      ...
      describe "#to_yaml" do
        it "round-trips through YAML.safe_load" do
          ...
          expect(parsed["language_code"]).to eq("eng")
          expect(parsed["sections"].first).to include(
            "number" => "1",
            "title" => "First",
          )
        end
