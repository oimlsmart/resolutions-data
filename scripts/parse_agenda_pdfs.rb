...
pdfs_by_slug.each do |slug, pdfs|
  ...
  identifier = [Edoxen::StructuredIdentifier.new(prefix: prefix, number: ordinal.to_s)]
  builder = Oiml::ResolutionsData::AgendaBuilder.new(meeting_urn: meeting_urn, identifier: identifier)
  items.each do |it|
    builder.add_item(label: it["label"].to_s, title: it["title"], kind: it["kind"], outcome: it["outcome"])
  end

  out_path = File.join(OUT_DIR, "#{slug}.yaml")
  # Edoxen::Agenda doesn't carry a top-level urn field — patch it in
  # after serialization so build-data.mjs can derive the meeting slug.
  yaml = builder.to_yaml.sub(/\A---\s*\n/, "").lstrip
  yaml = yaml.sub(/\Aidentifier:/, "urn: #{meeting_urn}\nidentifier:")
