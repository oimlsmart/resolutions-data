...
by_meeting.each do |(ordinal, lang), info|
  ...
  identifier = [Edoxen::StructuredIdentifier.new(prefix: "CIML", number: ordinal.to_s)]
  builder = Oiml::ResolutionsData::MinutesBuilder.new(
  ...
  sections.each do |s|
    builder.add_section(number: s["number"], title: s["title"], narrative: s["narrative"])
  end

  out_path = File.join(OUT_DIR, "ciml-#{ordinal}-#{lang}.yaml")
  File.write(out_path, "---\n# Auto-generated by scripts/parse_minutes.rb.\n" +
                       builder.to_yaml.sub(/\A---\s*\n/, "").lstrip)
  written << out_path
